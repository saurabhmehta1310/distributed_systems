...
def heartbeat():
    ...
    def heart(node_no):
        ...
        if len(log)>1:
            request.prevLogTerm=log[len(log)-2][2]
        n=nextIndex[node_no]
        ...
        while(n<=l):
            ...
            if n>len(log):
                ...
                request.entries+=f"{log[n-1][0]},{log[n-1][1]},{log[n-1][2]},{log[n-1][3]} "
                n+=1
...
hb=True
def reqVote():
    global term,vote_casted,role,voted_for,node_num,t1,recv_beat
    ...
    if vote_casted:
        ...
        if votes>=3:
            ...
            role="follower"
            t1=threading.Timer(random.uniform(6,12),reqVote)
            votes=0
            vote_casted=False
            t1.start()
...
def serve():
    ...
    print('Starting server. Listening on port 50048')
